...
int tirage_SFR( const float* pos, int NPart, const float* P, const float* L, int NTirage, float rhoSeuilMin, float* posOut, unsigned long int seed, float* mass, float* weight ) {
    ...
    */
    ...
    for(int i=0;i<NPart;i++){
        ...
        if(P[i]>rhoSeuilMin) {
            float dv= pow(0.5,L[i]*3);
            lambda = ceil( (float)NTirage *dv * (P[i]) / (rhoMax) );
            //lambda = 1+  log(  pow(0.5,L[i]*3) * (float)NTirage  * P[i] / rhoMax );
            //nPoint = lambda;
        ...
        if(nPoint) {
            ...
            for(int p=cptTirage;p<(cptTirage+nPoint);p++){
                ...
                R = gsl_rng_uniform(foncRand)*dx*6;

                float x = pos[i*3+0] + R*sinPH*cos(TH);
                float y = pos[i*3+1] + R*sinPH*sin(TH);
                float z = pos[i*3+2] + R*cos( asin(sinPH) );

                if (x> 1) x-=1.;  if (x<0) x+=1.;
                if (y> 1) y-=1.;  if (y<0) y+=1.;
                if (z> 1) z-=1.;  if (z<0) z+=1.;
